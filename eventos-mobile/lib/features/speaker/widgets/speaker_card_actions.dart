...
class SpeakerCardActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isLoved = speakerDetail?.isLoved ?? speaker?.isLoved ?? false;
    final speakerId = speakerDetail?.id ?? speaker?.id;
    final speakerName = speakerDetail?.name ?? speaker?.name ?? 'Speaker';
    final speakerDesignation = speakerDetail?.designation ?? speaker?.designation ?? 'Guest Speaker';
    final speakerImage = speakerDetail?.image ?? speaker?.image ?? '';

    // Determine if a note exists
    final hasNoteLocally = speakerId != null && Get.find<BriefcaseController>().notes.any(
      (n) => n.noteType == 'Speaker' && n.entityId == speakerId,
    );
    final haveNotes = speakerDetail?.haveNotes ?? speaker?.haveNotes ?? hasNoteLocally;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          // Bookmark / loved toggle
          GestureDetector(
            onTap: () {
              if (speakerId != null) {
                final speakerCtrl = Get.find<SpeakerController>();
                speakerCtrl.toggleBookmark(speakerId);
              }
            },
            child: Container(
          ...
          Expanded(
            child: GestureDetector(
              ...
              child: Container(
                height: 42.sp,
                decoration: BoxDecoration(
                  color: haveNotes ? context.primaryFocused : context.tertiaryText,
                  borderRadius: BorderRadius.circular(8.r),
