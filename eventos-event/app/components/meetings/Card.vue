...
<template>
  <article class="card">
    ...
    <p v-if="meeting.title || meeting.agenda" class="agenda">{{ meeting.title || meeting.agenda }}</p>

    <div v-if="meeting.can_respond" class="acts">
      <button type="button" class="btn ghost" :disabled="acting" @click="store.respond(meeting, 'reject')">Decline</button>
      <button type="button" class="btn primary" :disabled="acting" @click="store.respond(meeting, 'accept')">Accept</button>
    </div>
    <div v-else-if="meeting.direction === 'outgoing' && meeting.status === 'requested'" class="acts">
      <button type="button" class="btn primary" :disabled="acting" @click="store.respond(meeting, 'cancel')">Withdraw</button>
    </div>
...
.badge.wait { background: #fff6e5; color: #d97706; }
.badge.muted { background: #f1f5f9; color: #64748b; }
.time { font-size: .84rem; font-weight: 600; color: #334155; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

.with { margin: 0 0 10px; font-size: .8rem; color: #94a3b8; }
.booth { margin-left: 6px; padding: 2px 7px; border-radius: 999px; font-size: .62rem; font-weight: 700; text-transform: uppercase; letter-spacing: .4px; color: var(--brand-primary); background: color-mix(in srgb, var(--brand-primary) 12%, #fff); }

.who { display: flex; align-items: center; gap: 12px; }
.avatar { width: 44px; height: 44px; border-radius: 50%; overflow: hidden; flex: 0 0 auto; background: color-mix(in srgb, var(--brand-primary) 12%, #fff); display: flex; align-items: center; justify-content: center; }
.avatar img { width: 100%; height: 100%; object-fit: cover; }
.meta { min-width: 0; }
.name { margin: 0; font-size: .95rem; font-weight: 700; color: #1e293b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.role { margin: 2px 0 0; color: #64748b; font-size: .8rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

.place { display: flex; align-items: center; gap: 8px; padding-top: 14px; border-top: 1px solid #eef0f3; font-size: .84rem; color: var(--brand-primary); }
.place svg { width: 18px; height: 18px; flex: 0 0 auto; fill: none; stroke: currentColor; stroke-width: 1.7; stroke-linecap: round; stroke-linejoin: round; }
.place span { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.place.empty { color: #94a3b8; }

.agenda { margin: 0; font-size: .86rem; color: #475569; line-height: 1.45; }

.acts { display: flex; justify-content: flex-end; gap: 10px; margin-top: auto; }
.btn { border: none; border-radius: 10px; padding: 10px 22px; font: inherit; font-size: .85rem; font-weight: 600; cursor: pointer; }
.btn:disabled { opacity: .6; cursor: default; }
.btn.primary { background: var(--brand-primary); color: #fff; }
.btn.ghost { background: #f1f5f9; color: #475569; }
.btn.ghost:hover:not(:disabled) { background: #fdecec; color: #dc2626; }
</style>
