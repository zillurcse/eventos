  ...
  <aside class="side">
    ...
    <nav class="list card">
      <NuxtLink to="/profile" class="row" :class="{ active: route.path === '/profile' }">
        <svg viewBox="0 0 24 24">
          <path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 3 21.5l1-4.5z" />
        </svg>
        Edit Profile
        <svg v-if="route.path === '/profile'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      </NuxtLink>
      <NuxtLink to="/profile/bookmarks" class="row" :class="{ active: route.path === '/profile/bookmarks' }">
        ...
        My Bookmarks
        <svg v-if="route.path === '/profile/bookmarks'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
        <span
          v-else-if="bookmarks.count('speaker') + bookmarks.count('session') + bookmarks.count('delegate') + bookmarks.count('exhibitor')"
          class="count">
          {{ bookmarks.count('speaker') + bookmarks.count('session') + bookmarks.count('delegate') +
            bookmarks.count('exhibitor') }}
        </span>
      </NuxtLink>
      <NuxtLink to="/profile/briefcase" class="row" :class="{ active: route.path === '/profile/briefcase' }">
        ...
        My Briefcase
        <svg v-if="route.path === '/profile/briefcase'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      ...
      <NuxtLink to="/profile/schedule" class="row" :class="{ active: route.path === '/profile/schedule' }">
        ...
        My Schedule
        <svg v-if="route.path === '/profile/schedule'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      </NuxtLink>
      <button type="button" class="row" disabled title="Coming soon">
        <svg viewBox="0 0 24 24">
          <path d="M4 4h16v12H4zM9 21h6M9 16v5" />
        </svg>
        My Certificates
      </button>
      <NuxtLink to="/profile/badge" class="row" :class="{ active: route.path === '/profile/badge' }">
        ...
        My Badge
        <svg v-if="route.path === '/profile/badge'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      </NuxtLink>
    </nav>
    ...
    <nav class="list card">
      <NuxtLink to="/profile/password" class="row" :class="{ active: route.path === '/profile/password' }">
        ...
        Change Password
        <svg v-if="route.path === '/profile/password'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      </NuxtLink>
      <NuxtLink to="/profile/notifications" class="row" :class="{ active: route.path === '/profile/notifications' }">
        ...
        Notifications
        <svg v-if="route.path === '/profile/notifications'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      </NuxtLink>
      <NuxtLink to="/profile/language" class="row" :class="{ active: route.path === '/profile/language' }">
        <svg viewBox="0 0 24 24">
          <path d="M11 19a8 8 0 1 0 0-16 8 8 0 0 0 0 16zM3 11h16M11 3a13 13 0 0 1 0 16 13 13 0 0 1 0-16z" />
        </svg>
        Language &amp; Time zone
        <svg v-if="route.path === '/profile/language'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      </NuxtLink>
      <NuxtLink to="/profile/sessions" class="row" :class="{ active: route.path === '/profile/sessions' }">
        ...
        Browser Session
        <svg v-if="route.path === '/profile/sessions'" class="chev" viewBox="0 0 24 24">
          <path d="M9 6l6 6-6 6" />
        </svg>
      </NuxtLink>
    </nav>
...
.row:not(:disabled):not(.active):hover {
  background: #f7f8fa;
}
