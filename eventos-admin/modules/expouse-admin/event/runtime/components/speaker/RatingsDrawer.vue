...
interface SpeakerRatingsSummary {
...
const ratings = ref<SpeakerRatingRow[]>([])
const ratingsSummary = ref<SpeakerRatingsSummary>({
  ratings_count: 0,
  average_score: null,
...
async function loadRatings(force = false) {
  if (ratingsLoading.value) return
  ratingsLoading.value = true
  try {
    const res = await api<{
      data: { ratings?: SpeakerRatingRow[], summary?: SpeakerRatingsSummary }
    }>(`/events/${props.eventId}/speakers/${props.speakerId}/ratings`)
    ratings.value = res.data?.ratings ?? []
    ratingsSummary.value = res.data?.summary ?? ratingsSummary.value
  } catch (e: any) {
    toast.error(e?.data?.message || 'Could not load speaker ratings.')
  } finally {
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="emit('close')">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
      <div class="flex items-start justify-between gap-4 px-6 py-5 border-b border-line">
        <div>
          <h3 class="font-semibold text-[1rem] text-ink m-0">Speaker ratings</h3>
          <p class="muted text-[.85rem] mt-1 mb-0">{{ speakerName }}</p>
        </div>
        <button type="button" class="btn ghost sm shrink-0" @click="emit('close')">Close</button>
      </div>
      ...
      <div class="px-6 py-5 overflow-y-auto">
        <div class="flex items-start justify-between gap-4 mb-4">
          <div>
            ...
            <p class="muted text-[.8rem] mt-1 mb-0">
              See who rated this speaker and how the scores are distributed.
            </p>
          </div>
        ...
        <div class="border border-line rounded-xl overflow-hidden">
          ...
          <div v-else-if="!ratings.length" class="px-4 py-8 text-center muted text-[.84rem]">
            No one has rated this speaker yet.
          </div>
