    ...
    >
      ...
      <template #actions="{ row }">
        <div class="relative inline-block" @click.stop>
          ...
          <div v-if="actionsOpenId === row.id" class="absolute right-0 top-full mt-1 bg-white border border-line rounded-xl shadow-lg z-20 min-w-44 overflow-hidden">
            <button class="w-full text-left px-4 py-2.5 text-[.88rem] hover:bg-[#f7f8fa] text-ink transition-colors" @click="openEdit(row); actionsOpenId = null">Edit</button>
            <button class="w-full text-left px-4 py-2.5 text-[.88rem] hover:bg-[#f7f8fa] text-ink transition-colors" @click="toggleStatus(row)">
              {{ (row.status || 'active') === 'active' ? 'Deactivate' : 'Activate' }}
            </button>
            <button class="w-full text-left px-4 py-2.5 text-[.88rem] hover:bg-[#f7f8fa] text-ink transition-colors" @click="openResetPassword(row)">Reset Password</button>
            <button class="w-full text-left px-4 py-2.5 text-[.88rem] hover:bg-[#f7f8fa] text-[#dc2626] transition-colors" @click="remove(row); actionsOpenId = null">Delete</button>
          </div>
        </div>
      </template>
    </DataTable>
